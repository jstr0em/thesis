...
In our model we will assume that $G = 0$ as the groundwater is the sole contaminant source, i.e. there are no other sources of TCE in the soil, and TCE also does not readily degrade in soils. % TODO: Source for TCE degradation
...
\begin{equation}\label{eq:henrys_law}
  c_g = K_H c_w
\end{equation}
where, for example $K_H = 0.402$ is the dimensionless Henry's Law constant for TCE at \SI{20}{\degreeCelsius}. % TODO: TCE K_H source
...
Here either the gas-solid or water-solid sorption interaction can be chosen; the former is used in Chapter (TBD) where we will explore effect of gas-solid sorption. % TODO: Chapter reference
\begin{equation}
  c_s = \begin{cases}
...
\end{enumerate}
Regardless, we will continue carrying the sorption $K_p$ term, because this will become relevant in Chapter (TBD) where experimentally derived relevant sorption data are available.\par % TODO: Chapter reference
...
Another popular approach is Bruggeman's model.\par % TODO: Add Bruggeman's reference here
...
\begin{align}
  ...
  &\text{Groundwater} & c_w = c_{gw} = \SI{0.1}{\mol\per\metre\cubed} \\ % TODO Change value to something better, lower.
